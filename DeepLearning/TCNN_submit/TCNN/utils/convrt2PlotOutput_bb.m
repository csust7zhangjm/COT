function [ output_args ] = convrt2PlotOutput( seqList )
%CONVRT_OUTPUT Summary of this function goes here
%   Detailed explanation goes here

seqList = importdata('./datasets/seq_list_test.txt');
methodList = {'tnet_bbreg6_vgg-m1_new_random'};

for i=1:length(seqList)
    for j=1:length(methodList)
        resultDir = fullfile(pwd,'plotResult',[methodList{j},'_bb']);
        if ~exist(resultDir, 'dir'), mkdir(resultDir); end
        
        seqName = seqList{i};
        config = genConfig(seqName, methodList{j});
        
        switch(seqName)
            case{'FaceOcc1'}
                seqName = 'Faceocc1';
            case{'FaceOcc2'}
                seqName = 'Faceocc2';
            case{'FleetFace'}
                seqName = 'Fleetface';
        end
        resultName = [lower(seqName(1)) seqName(2:end) '_' methodList{j} '_bb.mat'];
        len = length(config.Dir.imgs);
        
        bboxPath = fullfile(config.Dir.result, 'result_bb.mat');
        if(~exist(bboxPath,'file'))
            fprintf('%s does not exist\n',bboxPath);
            continue;
        end
        
        res = load(bboxPath);
        
        results = cell(1);
        results{1}.res = res.result_bb;
        results{1}.type = 'rect';
        results{1}.len = len;
        switch(config.Name.seq)
            case{'David'}
                results{1}.annoBegin = 300;
                results{1}.startFrame = 300;
            case{'Tiger1'}
                results{1}.annoBegin = 1;
                results{1}.startFrame = 6;
            otherwise
                results{1}.annoBegin = 1;
                results{1}.startFrame = 1;
        end
        save(fullfile(resultDir,resultName), 'results');
    end
end


end

